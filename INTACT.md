...
1. Open the **Chat** tab and tell Shannon what you need — e.g. *"I need renter's insurance for my
...
3. For a car quote it asks the right accident question — **at-fault or not, and roughly when** —
...
6. If the record is high-risk (or you'd rather talk to someone), Shannon **doesn't invent a price** —
...
- Rich in-chat **quote cards** — estimate range, monthly/annual + payment plan, coverage, and the
...
- **Knows when *not* to quote:** high-risk records (multiple recent at-fault, a DUI) are routed to
  a broker instead of a made-up price — an **appetite check**.
