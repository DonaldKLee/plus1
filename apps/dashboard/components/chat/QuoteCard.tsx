...
import { Car, Home, Check, Alert } from "@/components/icons";
...
export function QuoteCard({ q }: { q: QuoteResult }) {
  ...
  const broker = q.appetite !== "standard";
  ...
  return (
    <div className="overflow-hidden rounded-[var(--r-lg)] border border-border bg-bg-subtle">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-2.5">
        ...
        {broker ? (
          <Chip color="var(--alert)">
            <Alert width={11} height={11} /> needs a broker
          </Chip>
        ) : (
      ...
      {broker ? (
        /* high-risk / refer: lead with the handoff, not a price */
        <div className="px-4 py-4">
          <p className="text-[14px] font-medium text-fg">This one’s best placed by a broker.</p>
          {q.handoffReason && (
            <p className="mt-1.5 text-[13px] leading-relaxed text-fg-muted">{q.handoffReason}</p>
          )}
          <p className="mt-3 text-[12.5px] text-fg">
            <span className="text-fg-muted">Intact broker line:</span>{" "}
            <span className="tnum font-medium">1-866-464-2424</span>
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="px-4 pt-4 pb-3">
            ...
            {/* payment */}
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 rounded-[var(--r-sm)] bg-bg-inset px-3 py-2 text-[12px]">
              <span className="text-fg-muted">
                Pay in full <span className="tnum font-medium text-fg">${money(q.payment.annual)}/yr</span>
              </span>
              <span className="text-fg-muted">
                or <span className="tnum font-medium text-fg">${q.payment.monthly}/mo</span>
                <span className="text-fg-subtle"> (+{Math.round(q.payment.instalmentFeePct * 100)}% instalment)</span>
              </span>
            </div>
            <p className="mt-1.5 text-[11.5px] text-fg-subtle">{q.coverageTier}</p>
          </div>
