...
/* Counts come from the demo fixture at build time — no invented statistics.
   Latency is deliberately NOT derived from the fixture: the classifier is not
   built yet, so its hop timings are authored JSON, not an instrument reading. */
const gated = fixture.events.filter((e) => e.kind === "utterance" && e.gate);
const acted = gated.filter((e) => INTENT_META[e.gate!.intent].act);
        ...
        <div className="mx-auto max-w-[1200px] px-5 py-20 sm:px-8 sm:py-28">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-20">
            ...
            <p className="mt-4 max-w-[58ch] text-[12.5px] leading-snug text-fg-subtle">
              The counts are read out of the demo fixture at build time. The
              latency is the budget the gate is being designed to, not a
              measurement — the classifier is still being built, and we would
              rather tell you that than quote you a number from a fixture.
            </p>
